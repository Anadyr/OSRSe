...
public class TextUtilities {
     ...
     * @return
     */
    public static String makeAddress(byte dataCheck, int b1, int b2, int b3, int b4) {
        StringBuilder build = new StringBuilder();
        build.append((b1 + ((dataCheck & 0x1) != 0 ? 129 : 0))).append(".").append((b2 + ((dataCheck & 0x4) != 0 ? 129 : 0))).append(".")
                .append((b3 + ((dataCheck & 0x8) != 0 ? 129 : 0))).append(".").append((b4 + ((dataCheck & 0x10) != 0 ? 129 : 0)));
        return build.toString();

    }

    /**
     * Turns a 4 short address into 5 bytes.
     * 4 * 2 = 8
     * 4 + 1 = 5
     * 3 byte difference. Strings would be even more.
     *
     * @return
     */
    public static byte[] makeAddress(String address) {
        byte[] fin = new byte[5];
        int make = 0;
        String[] l = address.split("\\.");
        int[] k = new int[4];
        for (int i = 0; i < 4; i++) {
            k[i] = Integer.parseInt(l[i]);
            if (k[i] >= 128) {
                k[i] -= 129;
                switch (i) {
                    case 0:
    ...
    public static int daysBetween(Date d1) {
        return (int) ((cal.getTime().getTime() - d1.getTime()) / (1000 * 60 * 60 * 24));
    }


    public static int getIPHash(String ip) {
        String[] ipSplit = ip.split("\\.");
        return (Integer.parseInt(ipSplit[0]) << 24 | Integer.parseInt(ipSplit[1]) << 16 | Integer.parseInt(ipSplit[2]) << 8 | Integer.parseInt(ipSplit[3]));
    }
    ...
    public static String readGJString2(byte[] buffer, int offset, int length) {
        char[] var4 = new char[length];
        int var5 = 0;
        int var7 = offset;

        int var8;
        for(int var9 = length + offset; var7 < var9; var4[var5++] = (char)var8) {
            int var6 = buffer[var7++] & 255;
            if(var6 < 128) {
                if(var6 == 0) {
                    var8 = '\ufffd';
                } else {
                ...
                if(var6 < 240) {
                    if(1 + var7 < var9 && (buffer[var7] & 192) == 128 && 128 == (buffer[var7 + 1] & 192)) {
                        var8 = (var6 & 15) << 12 | (buffer[var7++] & 63) << 6 | buffer[var7++] & 63;
                        if(var8 < 2048) {
                            var8 = '\ufffd';
                        }
                    } else {
                    ...
                    if(var7 + 2 < var9 && (buffer[var7] & 192) == 128 && 128 == (buffer[1 + var7] & 192) && (buffer[var7 + 2] & 192) == 128) {
                        var8 = (var6 & 7) << 18 | (buffer[var7++] & 63) << 12 | (buffer[var7++] & 63) << 6 | buffer[var7++] & 63;
            ...
            } else if(var7 < var9 && 128 == (buffer[var7] & 192)) {
                var8 = (var6 & 31) << 6 | buffer[var7++] & 63;
                if(var8 < 128) {
                    var8 = '\ufffd';
                }
            } else {

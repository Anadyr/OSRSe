...
public class ArrayUtilities {
    ...
    public static Integer[] removeJunk(int id, int[] array) {
        ...
        Integer[] okk = new Integer[ok.size()];
        ok.toArray(okk);
        return okk;
    }

    public static Short[] removeJunk(int id, short[] array) {
        ArrayList<Short> ok = new ArrayList<Short>();
        for(short i : array) {
            if(i != id)
                ok.add(i);
        }
        Short[] okk = new Short[ok.size()];
        ok.toArray(okk);
        return okk;
    }
    ...
    public static int[] removeValue(int[] array, int value) {
        ...
        for (int i = 0; i < 2; i++) {
            if (i == 0) {
                ...
                int curr = 0;
                finishedArray = new int[trueSize];
                for (int i3 = 0; i3 < array.length; i3++) {
                    if (array[i3] != value) {
                        finishedArray[curr] = array[i3];
                        curr += 1;
                    }
                }
            }

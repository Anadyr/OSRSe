 ...
 * @author Lazaro
 */
public class JSManager extends ImporterTopLevel {
	...
	public JSManager(File scriptDir) {
		...
		defineFunctionProperties(names, JSManager.class, ScriptableObject.DONTENUM);
		loopDir(ctx, scriptDir);
		System.out.println("Loaded " + scriptMap.size() + " script(s)");
		Context.exit();
	}
			...
			for (File f : dir.listFiles()) {
				if (f.isFile() && f.getName().endsWith(".js")) {
					...
					try {
						...
						System.err.println("Error loading script : " + name + ", " + e.getMessage());
					}
	...
	public void trace(Object msg, Object o) {
		StackTraceElement caller = Thread.currentThread().getStackTrace()[9];
		Logger logger = Logger.getLogger(caller.getFileName());
		logger.log(Level.SEVERE, msg.toString(), new Throwable(o.toString()));
	}

	public int random(int startRange, int endRange) {
		return Module.random(startRange, endRange);
	}

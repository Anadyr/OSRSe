        ...
        if (pathRequest != null) {
            ...
            Runnable r = new Runnable() {
                public void run() {
                    ...
                    final LinkedList<Tile> path = request.pathFinder.findPath(entity.getLocation(), srcX, srcY, destX, destY, true, type, direction, sizeX, sizeY, walkToData, !entity.basicSettings().isClipping(), entity.getSize());

                    if (request.pathFinder.isSuccessful()) {
                        if (!request.automated) {
                            entity.resetEvents(false);
                        }
                        if (request.future != null) {
                            request.future.run();
                        }
                        if (coordinateFuture != null) {
                            coordinateFuture.setReady(true);
                        }
                        moveSpeed = request.moveSpeed;
                    }
                    if (path != null) {
                        //TODO
                        if (request.pathFinder.movedNear() && (!(request.target instanceof Mob) /*|| !((Mob) request.target).getDefinition().name.equalsIgnoreCase("Banker") */ || path.getLast().distance(request.target.getLocation()) > 2)) {
                        ...
                        if (request.pathFinder.isSuccessful() && (!walkingToObject || !request.pathFinder.movedNear())) {
                            try {
                                ...
                                System.err.println("Error executing coordinate future!" + e.getMessage());
                            }
                        } else {

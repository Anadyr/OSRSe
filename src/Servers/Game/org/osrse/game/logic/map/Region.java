...
import org.osrse.game.file.parsers.MapDefinitionParser;
...
public final class Region {
    ...
    private static final MapDefinitionParser MAP_LOADER = new MapDefinitionParser();
    ...
    public void load() {
        if (!loaded) {
            ...
            try {
                ...
                System.err.println("Error loading map for region [x= " + x + ", y=" + y + "]!" + e.getMessage());
                e.printStackTrace();
            }
                try {
                    //Static.world.getDoorManager().loadRegion(this);
                } catch (Exception e) {
                    System.err.println("Error loading doors for region [x= " + x + ", y=" + y + "]! "+ e.getMessage());
    ...
    public void addObject(int objectId, int x, int y, int z, int type, int direction, boolean ignoreObjects) {
       // init(); was called too much when loading regions, re-add if it fucks up anything.
    ...
    public Region[] getSurroundings() {
        return new Region[] {
            forRegionCoordinates(x + 1, y),
            forRegionCoordinates(x, y - 1),
            forRegionCoordinates(x, y + 1),
            forRegionCoordinates(x + 1, y + 1),
            forRegionCoordinates(x + 1, y - 1),
            forRegionCoordinates(x - 1, y + 1),
            forRegionCoordinates(x - 1, y - 1),
            forRegionCoordinates(x - 1, y)
